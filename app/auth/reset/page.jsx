  ...
  return (
    <main className="container py-10">
      ...
      <form onSubmit={submit} className="mt-8 flex flex-col gap-4">
        {!token && <FormInput placeholder="Token (dal link email)" value={token} onChange={e=>setToken(e.target.value)} />}
        <PasswordInput placeholder="Nuova password" value={password} onChange={e=>setPassword(e.target.value)} />
        <Button type="submit" loading={loading} className="bg-white text-black w-full">Aggiorna password</Button>
      </form>
    </main>
  );
}
